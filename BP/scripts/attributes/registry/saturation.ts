...
class SaturationAttribute extends BaseAttribute {
    public id = "saturation";
    public runId = -1;
    ...
    public setValues(score: ScoreboardObjective): void {
        for (const player of world.getAllPlayers()) {
            const saturation = player.getComponent("player.saturation")?.currentValue ?? 0;

            score.setScore(player, saturation);
        }
    }
