...
const allowedSelectors = new Set(["s", "p", "a", "r", "e"]);
        ...
        for (let i = 0; i < codeLines.length; i++) {
            ...
            for (let j = 0; j < args.length; j++) {
                if (args[j].startsWith("@")) {
                    ...
                    if (allowedSelectors.has(method)) continue;
                    ...
                    if (parts.length > 2 && actorActions.has(parts[1])) {
                        ...
                        argMetaData[j].isCode = true;
                    }
                }
            }
            console.warn(JSON.stringify(argMetaData));
            ...
            if (action) {
                ...
                if (argMetaData.some((meta) => meta.processed || meta.isCode)) {
                    let processedArgs = [];
                    for (let i = 0; i < args.length; i++) {
                        if (argMetaData[i]?.processed || argMetaData[i]?.isCode) {
